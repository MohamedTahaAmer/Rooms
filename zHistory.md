...
```js
// 1- allow for subreddit name change
// 2- allow for post edit
// 3- give roles, like the creator of a subreddit can make some others admins, and the admins can't make others admins, but they can remove users from the subreddit, and if the user is removed, he can't rejoin the subreddit without aproval from one of the admins
// 4- make it when you click on the user's name, you get all of his posts in this subreddit
// 5- the way we count the the net votes count is using reducer, change it, store the upcount and the dwon count, then when you wanna know the net count, subtract the two
// 6- add optimistic update for comments
// 7-
```
